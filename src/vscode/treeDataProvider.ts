...
export class OrderedFileTreeDataProvider
    implements vscode.TreeDataProvider<vscode.TreeItem>
{
    ...
    getChildren(element?: vscode.TreeItem): Thenable<vscode.TreeItem[]> {
        ...
        if (element) {
            ...
            if (
                ...
                const orderedDirectory = this.orderProcessor
                    ? this.orderProcessor.orderFiles(directory)
                    : directory;
                return Promise.resolve(
                    orderedDirectory.children.map(toTreeItem)
                );
            }
            return Promise.resolve([]);
        } else {
            // Get root level files
            const directory = toFileSystemItem(
    ...
    private loadOrderFile(): void {
        ...
        if (fs.existsSync(orderFilePath)) {
            ...
            if (orderFile) {
                this.orderProcessor = new FileOrderProcessor(
                    orderFile,
                    path,
                    fs
                );
