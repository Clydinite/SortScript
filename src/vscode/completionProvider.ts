...
export class OrderCompletionProvider implements vscode.CompletionItemProvider {
  ...
  private getFilePatternCompletions(document: vscode.TextDocument, _position: vscode.Position): vscode.CompletionItem[] {
    ...
    try {
      ...
      const globPatterns = [
        ...
        { name: 'src/*.js', detail: 'All JavaScript files in the src directory' },
        { name: 'src/*.ts', detail: 'All TypeScript files in the src directory' },
...
export class OrderHoverProvider implements vscode.HoverProvider {
  ...
  private getDirectiveDocumentation(directiveName: string): string | null {
    const docs: { [key: string]: string } = {
      ...
      'allow_if': '**@allow_if** - Allow files only if they match the specified regex pattern.\n\nExample: `*.js @allow_if(/^[a-z]/)`',
      'disallow_if': '**@disallow_if** - Disallow files if they match the specified regex pattern.\n\nExample: `*.js @disallow_if(/\\.test\\./)`',
      ...
      'group_by': '**@group_by** - Groups files together based on basename or capture groups.\n\nExample: `@group_by(basename)` or `@group_by($1)`',
