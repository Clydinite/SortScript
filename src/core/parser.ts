...
export class OrderFileInterpreter {
    ...
    visitOrderFile(cstNode: CstNode): OrderFile {
        const statements =
            cstNode.children.statement?.map((stmt) =>
                this.visit(stmt as CstNode)
            ) || [];
        return { statements };
    }
    ...
    visitStatement(cstNode: CstNode): Statement {
        if (cstNode.children.pathBlock) {
            ...
            return this.visit(
                cstNode.children.filePattern[0] as CstNode
            ) as any;
        } else if (cstNode.children.directive) {
            return {
                type: "directive",
                directive: this.visit(
                    cstNode.children.directive[0] as CstNode
                ) as any,
            };
        } else if (cstNode.children.directiveBlock) {
            return this.visit(
                cstNode.children.directiveBlock[0] as CstNode
            ) as any;
        }
    ...
    visitPathBlock(cstNode: CstNode): Statement {
        ...
        const blockDirectives =
            cstNode.children.directive?.map(
                (dir) => this.visit(dir as CstNode) as any
            ) || [];
        const statements =
            cstNode.children.statement?.map(
                (stmt) => this.visit(stmt as CstNode) as any
            ) || [];
        return {
            type: "pathBlock",
            pattern,
    ...
    visitFilePattern(cstNode: CstNode): Statement {
        const pattern = this.visit(cstNode.children.pattern[0] as CstNode);
        const directives =
            cstNode.children.directive?.map((dir) =>
                this.visit(dir as CstNode)
            ) || [];
        return {
            type: "filePattern",
            pattern,
    ...
    visitDirective(cstNode: CstNode): Directive {
        ...
        const args =
            cstNode.children.directiveArg?.map((arg) =>
                this.visit(arg as CstNode)
            ) || [];
        return { name, args };
    }
